...
17. ~~Add handedness splits using the PBP data.~~ FIXED: PBP-derived vsL/vsR splits already calculated by calculate_advanced_stats.py; fixed frontend merge bug that was dropping battingSplits/pitchingSplits during monthly stats aggregation
...
24. ~~Add Pull-Air% to rolling charts (PlayerCharts.tsx BATTER_METRICS)~~ FIXED: Added Pull-Air% as a BIP-weighted metric to BATTER_METRICS in PlayerCharts.tsx
25. ~~Barrel% is not calculating correctly for Statcast stats~~ FIXED: Savant's minor league CSV doesn't populate the `barrel` field; replaced with EV/LA-based barrel computation using the Statcast barrel definition (98+ mph, 26-30° LA expanding ±1° per mph above 98, capped at 8-50°). Requires re-fetching Statcast data to recalculate.
26. ~~Players who played at multiple levels who have Statcast stats at one of those levels have the stats filled down rather than showing NA for levels that dont have Statcast data (for example, Trey Gibson pitched at A+, AA, and AAA in 2025. he has Statcast data from his AAA time which is showing correctly, but those numbers are copied on the AA and A+ lines even though Statcast data isnt available at that level)~~ FIXED: Added case-insensitive level matching and Savant level code mapping (Savant 'a' → 'A+' for FSL); statcast data now only displays on the row matching its source level
