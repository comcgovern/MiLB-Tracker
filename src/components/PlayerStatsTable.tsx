                ...
                return (
                  ...
                  >
                    ...
                    {activeCategory === 'arsenal' ? (
                      ...
                      <td colSpan={statColumns.length || 10} className="px-2 py-1">
                        {(() => {
                          const arsenal = (players.find(p => p.teamPlayer.id === teamPlayer.id)?.statcast?.pit?.arsenal);
                          if (arsenal && isFirstRowForPlayer) {
                            return <ArsenalTable arsenal={arsenal} playerName={player?.name || 'Unknown'} />;
                          }
                          if (isFirstRowForPlayer) {
                            return <span className="text-gray-400 dark:text-gray-500 italic text-sm">No arsenal data available</span>;
                          }
                          return null;
                        })()}
                      </td>
                      ...
                      statColumns.map((col) => {
                        // For the statcast category, try to pull Savant-specific metrics from
                        // the statcast block, falling back to PBP-derived stats in regular stats
                        ...
                        const rowStatcast = players.find(p => p.teamPlayer.id === teamPlayer.id)?.statcast;
